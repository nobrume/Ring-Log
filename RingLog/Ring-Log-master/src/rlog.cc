...
#define MEM_USE_LIMIT (1u * 1878 * 1024 * 1024)// windows single win32 exe toplimit is 1.97G 
...
pthread_t  thdo_tid;
int	pExit = 0;//exception exit flag
// pid_t gettid()
// {
...
// 	{
// 		ring_log::ins()->try_append("[ERROR]", "[%u]%s:%d(%s): %s \n", gettid(), __FILE__, __LINE__, __FUNCTION__, fmt, args);
// 	}
// 	va_end(args);
// }
...
void ring_log::persist()
{
    while (true)
    {
        ...
        if (_prst_buf->status == cell_buffer::FREE)
        {
			...
			tt = (li.QuadPart - EPOCHFILETIME) / 10 / 1000000;
			localtime_s(&now, (time_t*)&tt);
			
            tsp.tv_sec = now.tm_sec;
            tsp.tv_nsec = 0;//nanoseconds
        ...
        if (_prst_buf->empty())
        {
			...
			if (pExit){ pthread_mutex_unlock(&_mutex); break; }
			pthread_mutex_unlock(&_mutex);
            continue;
        }
...
void ring_log::try_append(const char* lvl, const char* format, ...)
{
    ...
    int prev_len = sprintf(log_line, "%s[%s.%03d]", lvl, _tm.utc_fmt, ms);//title+timeline
    ...
    uint32_t len = prev_len + main_len;//total len 

    _lst_lts = 0;
    bool tell_back = false;

    pthread_mutex_lock(&_mutex);
	//TO Append if there are enough size in buffer
    ...
    else//not enough
    {
        ...
        if (_curr_buf->status == cell_buffer::FREE)
        {
            ...
            if (next_buf->status == cell_buffer::FULL)
            {
                ...
                {
                    fprintf(stderr, "no more log space can use in Memory\n");
                    //_curr_buf = next_buf;// 1. buffer new memory is not deleted!. 2. curr_buffer content is not save to file but point changed.
					_curr_buf->status = cell_buffer::FULL;//2019.12.5
					pExit = 1;//2019.12.5

                    _lst_lts = curr_sec;
                }
                else//insert new buffer  in buffer loop
                {
...
bool ring_log::decis_file(int year, int mon, int day)
{
    ...
    if (!_fp)
    {
        ...
        char log_path[1024] = {};
        sprintf(log_path, "%s/%s.%d%02d%02d.log", _log_dir, _prog_name, _year, _mon, _day/*, _pid*/);
    ...
    else if (ftell(_fp) >= LOG_USE_LIMIT)
    {
        ...
        {
            sprintf(old_path, "%s/%s.%d%02d%02d.log.%d", _log_dir, _prog_name, _year, _mon, _day/*, _pid*/, i);
            sprintf(new_path, "%s/%s.%d%02d%02d.log.%d", _log_dir, _prog_name, _year, _mon, _day/*, _pid*/, i + 1);
            rename(old_path, new_path);
        }
        //mv xxx.log xxx.log.1
        sprintf(old_path, "%s/%s.%d%02d%02d.log", _log_dir, _prog_name, _year, _mon, _day/*, _pid*/);
        sprintf(new_path, "%s/%s.%d%02d%02d.log.1", _log_dir, _prog_name, _year, _mon, _day/*, _pid*/);
        rename(old_path, new_path);
